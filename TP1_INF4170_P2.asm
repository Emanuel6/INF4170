...
done1:	add	$a0,$0,$s0
	...
	li	$v0, 4
	la 	$a0, msg_tri	# Affichage des éléments triés
	syscall
	add	$s3, $0, $0
show:	lw	$s4, 0($s0)
	li	$v0, 1
	...
	beq	$s3, $s2, end
	li	$v0, 4
	la 	$a0, msg_virg 	# Affichage d'une virgule après chaque élément sauf le dernier.
	syscall
	j	show
	syscall
...
# du heapsort
sort:	addi 	$sp, $sp, -28 	# Sauvegarde des registres utilisés dans la fonction sort
	sw 	$a0, 0($sp)
	sw 	$a1, 4($sp)
	sw 	$ra, 8($sp)
	add	$t0, $0, $a1
...
for:	sge	$t2, $t1, $0	# for (int i = (n - 1) / 2; i >= 0; i--)
	...
	add	$a1, $0, $t1
	add	$a2, $0, $t0
	sw 	$t0, 12($sp)	# Sauvegarde des registres utilisés avant l'appel de la fonction fix
	sw 	$t1, 16($sp)
	sw 	$t2, 20($sp)
	sw 	$t3, 24($sp)
	jal	fix		# fixHeap(tableau = $t3, i = $t1, n = $t0)
	lw 	$t3, 24($sp)	# Chargement des registres sauvegardés
	lw	$t2, 20($sp)
	lw 	$t1, 16($sp)
	lw 	$t0, 12($sp)
	subi	$t1, $t1, 1	# i = i - 1
...
finFor: sgt	$t2, $t0, $0	# while (n > 0)
	...
	add 	$a0, $0, $0
	add	$a1, $0, $t0
	sw 	$t0, 12($sp)	# Sauvegarde des registres utilisés avant l'appel de la fonction swap
	sw 	$t1, 16($sp)
	sw 	$t2, 20($sp)
	sw 	$t3, 24($sp)
	...
	lw 	$t0, 12($sp)
	j	finFor
finSort:lw 	$ra, 8($sp)	# Chargement des registres sauvegardés
	lw 	$a1, 4($sp)
	lw 	$a0, 0($sp)
	addi 	$sp, $sp, 28
	jr	$ra
...
# haut du tas
fix:	addi 	$sp, $sp, -40	# Sauvegarde des registres utilisés dans la fonction fix
	sw	$ra, 0($sp)
	sw	$a0, 4($sp)
	sw 	$s0, 8($sp)
...
while:	bne	$t3, 1, finFix
	sw 	$t0, 12($sp)	# Sauvegarde des registres utilisés avant l'appel de la fonction gLci
	sw 	$t1, 16($sp)
	sw	$t2, 20($sp)
	sw	$t3, 24($sp)
	add	$a0, $0, $t1
	...
	beq	$t6, 1, fWhile	# Si childIndex > lastIndex, on met fin à la boucle
	sll	$t4, $t4, 2
	add 	$t0, $t0, $t4
	lw	$t7, 0($t0)	# $t7 = a[childIndex]
	sub	$t0, $t0, $t4
	srl	$t4, $t4, 2
	sw 	$t7, 12($sp)	# Sauvegarde des registres utilisés avant l'appel de la fonction gRci
	sw 	$t6, 16($sp)
	sw	$t5, 20($sp)
	sw	$t4, 24($sp)
	sw 	$t3, 28($sp)
	...
	lw	$t0, 40($sp)	# Chargement des registres sauvegardés
	lw	$t1, 36($sp)
	lw 	$t2, 32($sp)
	lw 	$t3, 28($sp)
	lw	$t4, 24($sp)
...
lci:	sll	$t4, $t4, 2
	...
	bne	$s0, 1, fWhile	# si a[childIndex] <= rootValueon met fin à la boucle
	sll	$t1, $t1, 2
	add 	$t0, $t0, $t1
...
finFix: sll	$t1, $t1, 2
	...
	sub	$t0, $t0, $t1
	srl	$t1, $t1, 2
	lw 	$s0, 8($sp)	# Chargement des registres sauvegardés
	lw	$a0, 4($sp)
	lw	$ra, 0($sp)
	addi 	$sp, $sp, 40
	jr 	$ra
